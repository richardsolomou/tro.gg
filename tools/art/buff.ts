...
export function buffDraw(p: PixelSink, view: View, frame: FrameName): void {
  ...
  if (view === "up") {
    ...
    shaded(p, 6, 31 + b, 2.8, 3.6, c.skin, c.skinDk);
    shaded(p, 25, 31 + b, 2.8, 3.6, c.skin, c.skinDk);
    return;
  }
  ...
  if (view === "side") {
    ...
    shaded(p, 18, 26 + b, 2.8, 3, c.skin, c.skinDk); // near upper arm
    shaded(p, 20, 31 + b, 2.6, 2.8, c.skin, c.skinDk); // near fist (~hog side hand 20,32)
    return;
  }
  ...
  for (const s of [-1, 1] as const) {
    const sx = 15.5 + s * 9;
    shaded(p, sx, 24 + b, 3, 3.6, c.skin, c.skinDk); // deltoid
    shaded(p, sx + s * 0.8, 29 + b, 2.6, 3, c.skin, c.skinDk); // upper arm
    shaded(p, 15.5 + s * 10.5, 33 + b, 2.6, 2.6, c.skin, c.skinDk); // fist
  }
