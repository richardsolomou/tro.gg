...
test("path motion never rewinds onto a blocked tile it has already crossed", () => {
  // A creature wanders onto (2,1) — a tile the trogg already walked over — after the route
...
test("a creature's walkable headings exclude walls and the zone edge", () => {
...
test("a creature's walkable headings treat an occupied tile (boulder/creature/trogg) like a wall", () => {
...
// --- Big (2×2) creature footprints: a size-2 mover clamps across its whole footprint ---
...
test("a 2×2 creature stops its right edge flush against a wall, a tile sooner than a 1×1", () => {
...
test("a 2×2 creature is clamped so its whole footprint stays inside the zone", () => {
...
test("a 2×2 creature's walkable headings test the whole leading edge", () => {
