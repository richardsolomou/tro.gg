 ...
 * to tiles. Tile-by-tile roaming creatures still walk with cardinal intents.
 *
...
export interface Motion {
   ...
   *  absent/false = walk. Tile-by-tile roaming creatures never set it, so they
   *  always walk. */
  running?: boolean;
  /** Tile-footprint span anchored at (x, y) as the top-left corner: 1 for a trogg
   *  or a common-sized roamer, 2 for a larger creature. Absent = 1. The footprint
 ...
 * Tile-by-tile roaming creatures and legacy rows use plain ±1 vectors, which
 * are just short headings.
 */
export const DIR_SCALE = 1000;

/** The four cardinal movement directions — the headings a tile-by-tile wander picks from. */
...
/**
 * The cardinal directions a tile-by-tile roaming creature at (x, y) could step
 * onto — where its whole `size`-tile footprint, shifted one tile that way, lands
 * on walkable floor inside the zone. A tile-by-tile wander reducer picks its next
 * heading from these, so it ambles around walls, boulders, troggs, and other
 * creatures (whatever the `ZoneBounds` `occupied` predicate marks unwalkable).
 * For a 2×2 creature the `occupied` predicate must exclude its own footprint,
 * else its next step (which overlaps where it stands) reads as blocked. (x, y)
 * is the footprint's top-left tile.
 */
...
function projectPathMotion(motion: Motion, path: readonly Coord[], elapsedMs: number, zone: ZoneBounds): Omit<ProjectedMotion, "z"> {
  ...
  for (const next of path) {
    ...
    if (remaining <= hop) {
      ...
      // obstacle that arrived after routing (a roaming creature, a shoved boulder)
      // clamps the glide instead of being passed through. A clamp or slide means
      // the hop is no longer clean — report no heading, which is the stall signal
      // the client re-routes on. Hops already consumed are behind us: forward-only
      // projection, so a creature landing on ground already covered never rewinds
      // the trogg.
