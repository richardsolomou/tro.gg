...
const hog = table(
  { name: "hog", public: true },
  {
    ...
    // Empty means "derive from id" for existing and registry-spawned Hogs; carried
    // Hogs store their effective style here on put-down so pickup/drop preserves it.
...
function effectiveHogStyle(h: { id: bigint; style?: string }): string {
  return h.style || hogStyleFor(h.id.toString());
}
