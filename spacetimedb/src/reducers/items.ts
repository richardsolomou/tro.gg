 ...
 * carrying a legacy boulder, `F` throws it as a tile-based impact weapon. Otherwise
 * the swing resolves in order: the weapon's own gathering node at full damage, a
 * creature, then any node at a fraction of the roll; at zero health the target dies
 * or the node breaks and deposits its resource into the stockpile.
 */
function runUseEquipped(ctx: Ctx, { dirX, dirY, source = "" }: { dirX: number; dirY: number; source?: string }): AnalyticsEvent[] {
  ...
  // A breaking hit never fills personal inventory: the yield deposits straight
  // into the shared stockpile (GDD "The fire and the dark"), no ground item and
  // no conscious pickup involved.
  const strikeBoulder = (b: NonNullable<ReturnType<typeof meleeBoulderTarget>>["target"], damage: number): boolean => {
    ...
    const qty = depositStockpile(ctx, "stone", 1);
    events.push({ distinctId: distinctId(ctx), event: "resource_gathered", properties: { ...props, node_type: "boulder", item: "stone", qty } });
    return true;
  };
  const strikeTree = (tr: NonNullable<ReturnType<typeof meleeTreeTarget>>["target"], damage: number): boolean => {
    ...
    const qty = depositStockpile(ctx, "wood", 1);
    events.push({ distinctId: distinctId(ctx), event: "resource_gathered", properties: { ...props, node_type: "tree", item: "wood", qty } });
    return true;
  };
  ...
  if (range) {
    ...
    if (!landed) {
      const damage = roll();
      // A dark creature outranks a trogg at equal distance (GDD "Dark
      // creatures" → "Combat") — the two are meant to be fighting side by
      // side, not clipping each other in the same swing.
      const trogg = meleePlayerTarget(ctx, p.zoneId, cx, cy, aim, ctx.timestamp, p.identity);
      const darkCreature = meleeDarkCreatureTarget(ctx, p.zoneId, cx, cy, aim);
      if (darkCreature && (!trogg || darkCreature.dist <= trogg.dist)) {
        const result = damageDarkCreature(ctx, darkCreature.target, damage);
