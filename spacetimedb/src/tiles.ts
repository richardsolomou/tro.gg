 ...
 * against everything solid to a trogg — walls and boulders — so it settles
 ...
 * *not* collide with each other, so other players are absent here.
 */
...
export function addPlayerTiles(ctx: Ctx, zoneId: string, now: Stamp, set: Set<string>, exclude?: Ctx["sender"]): void {
  ...
  if (!zone) return;
  const blockers = troggBlockers(ctx, zoneId);
  const bounds = zoneBounds(zone, (x, y) => blockers.has(tileKey(x, y)));
...
/** Materialise a carried entity on an exact tile, enforcing the same cap as
 *  put-down. Currently only the legacy "boulder" carry kind exists. */
export function placeCarriedAt(ctx: Ctx, zone: Zone, kind: string, _style: string, tile: { x: number; y: number }): boolean {
