...
Creature and item art is code-built 3D: jointed low-poly models and animation clips authored in `src/game/` (`creatures.ts`, `items.ts`, `rig.ts`, palettes in `palette.ts`) — no modelled assets, no editor files. Iterate with the dev preview: run `pnpm dev` and open `/preview` (URL-addressable states — creature, held items, anim mode, pause/scrub, bones overlay); HUD icons render from the same models (`icons.ts`). Concept references live in `docs/art-refs/`.
