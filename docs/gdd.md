...
- **Nodes have health and take swings** (`BOULDER_MAX_HEALTH` 45, `TREE_MAX_HEALTH` 54 *(initial)* — roughly three average hits each). Every tool swing that reaches its node rolls the tool's `WEAPON_DAMAGE` range into the node's health, and the client answers each hit with the same feedback a creature gives: a white hit flash and a floating damage number. The breaking hit deletes the row and grants the resource.
