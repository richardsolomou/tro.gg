...
- **Mine a boulder with a pickaxe.** Pressing `F` with a pickaxe equipped strikes the nearest boulder in melee reach along the aim (the same reach + swing-cone geometry as combat): the boulder row is deleted and one **Stone** lands in inventory. No free slot for a new stone stack → the boulder stays.
- **Fell a tree with an axe.** The exact mirror: `F` with an axe equipped fells the nearest tree in reach into one **Wood**. The axe shares the pickaxe's overhead `chop` wield class.
