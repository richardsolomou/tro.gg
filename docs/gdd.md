...
- Visible in-game HUD surfaces — chat history/input, the top-right account claim control, and the top-left icon toggle bar (Help `?`/`H`, Settings `O`, Appearance `P`, Inventory `I`) — are HTML/CSS overlays above the canvas; the pre-alpha **Commands drawer** (`` ` ``) is deliberately on the opposite side — a lone toggle in the top-right corner opening a full-height right-edge drawer — so debug chrome never crowds player controls, so the browser owns layout, text input, focus, IME, and resize. They use `pointer-events` so a click on a panel is consumed by the DOM and a click on open space falls through to the canvas (click-to-move). The toggle bar is an accordion: opening one menu closes the others, and `Esc` closes the open menu. World-space labels, health bars, and speech bubbles over troggs are camera-facing canvas sprites inside the 3D scene (`src/game/overlays.ts`), drawn depth-test-off so a wall never swallows a nameplate. Help lists only enabled controls, and the Commands drawer exposes only enabled debug tools.
...
- The current audio surface is event-driven: local trogg footsteps fire when movement starts and when the trogg crosses tiles; a moving boulder row (a throw landing) plays a settle hit and a felled tree plays the same crash; Hog heading changes may play a sparse, low-volume, distance-faded Hog snuffle after the initial subscription snapshot; chat, commands, command errors, and the ghost haunt play short UI/ghost cues.
- **The player owns the mix.** Every sound in the game belongs to exactly one of four categories — **Music** (the theme), **Footsteps** (troggs and Hogs), **World** (boulders, Hog snuffles, the ghost), **Interface** (chat, commands, errors) — mapped per cue in `src/audio.ts` (`CUE_CATEGORY`) and enforced by the type of `src/sound-settings.ts`, so a new cue must pick a slider. The Settings panel (`O`, top-left toggle bar) holds a 0–100% slider per category; levels apply live (the theme re-mixes without restarting) and persist in `localStorage`, which the landing page theme honours too.
...
- **Thrown Hogs:** pressing `F` while carrying throws the held object in a straight cardinal line up to `THROWN_OBJECT_RANGE` (4 tiles *(initial)*). It damages the first online, living trogg or Hog it hits by `THROWN_OBJECT_DAMAGE` (40 *(initial)*) and then lands near the impact; if it hits no damageable character, it lands at the farthest clear tile before range or a blocker. This is still an input-driven reducer, not a projectile simulation.
...
The arc: wake in the **middle of the glowmoss cavern** (a hearth ember lights the wake-up spot; the opening camera is raised and close), pick up the pickaxe beside you (`E`), equip (`I`), mine the **two rubble rocks** plugging the cavern end of a long throat (`F`), gather the stones they drop, and **walk the throat out — the walk is the door** (the transfer threshold sits at its far end, so nothing whisks you away before you've picked up your haul). Through the mouth you see the actual outside: the client renders a fog-free, daylight-lit window of the world map (the ~10 rows of coast in front of the arrival alcove, hard-clipped to the mouth's slot, composited from its OWN scene so its daylight never touches the cave's dark, and masked by throat-rock so no camera angle catches it floating), so emerging reads as walking outdoors, not switching instances. No props, no sparkle — an entrance. The `emerge` reducer verifies proximity server-side (invariant 3), **wipes the instance's rows** — it simply ceases — and transfers the trogg to `world` at `EMERGE_ARRIVAL`, a cave-mouth alcove carved into the south coast. The client reboots into the new zone behind the boot screen; the arrival opens on a raised reveal shot, fires `warren_emerged`, and plays the **first dawn** (~12s local-only blend from morning gold into the shared sky; the shared sky lock outranks it).
