...
The arc: wake in the **middle of the glowmoss cavern** (a hearth ember lights the wake-up spot; the opening camera is raised and close), pick up the pickaxe beside you (`E`), equip (`I`), mine the **two rubble rocks** plugging the neck up to the exit landing (`F`), and press **`E` beside the exit light** (a warm glow + the pickup-sparkle language). No cell, no maze — one lit room, one job, one door. The `emerge` reducer verifies proximity server-side (invariant 3), **wipes the instance's rows** — it simply ceases — and transfers the trogg to `world` at `EMERGE_ARRIVAL`, a cave-mouth alcove carved into the south coast. The client reboots into the new zone behind the boot screen; the arrival opens on a raised reveal shot, fires `warren_emerged`, and plays the **first dawn** (~12s local-only blend from morning gold into the shared sky; the shared sky lock outranks it).
