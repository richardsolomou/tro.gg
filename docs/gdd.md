...
- **Same motion model as troggs.** A Hog carries an intent — origin `(x, y)`, a cardinal direction, `movedAt`, and a click-to-move `path` — and clients derive its position with `projectMotion`, so it's never per-frame synced (invariant 2) and it collides against the same walls and boulders.
- **Driven by a scheduled reducer, not a tick.** A Hog gets a new destination only inside the scheduled `wanderHogs` reducer (SpacetimeDB's deterministic timer — the sanctioned exception in invariant 1, like respawns). It walks a route, not a single step: when a Hog reaches the end of its `path` (or stalls on a boulder dropped across it), the tick routes it with the same grid `A*` `findPath` as click-to-move (see "Movement") to a random reachable tile within `HOG_WANDER_RADIUS` of its **home** tile (where it entered the world), or idles it with chance `HOG_IDLE_CHANCE` so it pauses; an unreachable roll retries up to `HOG_WANDER_TRIES` times before idling. Anchoring destinations on home rather than the Hog's current spot keeps it in its patch instead of drifting off as each hop re-centres the radius. The route is stored as the Hog's `path` and clients glide along it via `projectMotion`, so a Hog still en route costs the tick nothing (the row is untouched until it arrives). Randomness is the context RNG (seeded from the tick timestamp), so the schedule replays deterministically (invariant 3).
