...
hog            id (PK, auto-inc), zoneId, x, y, dirX, dirY, movedAt, path, homeX, homeY, style
               ...
               kept only so the shipped schema isn't reordered. style: "" = common roamer (client-derived
               skin); "buff"/"dino" = a 2×2 showpiece; "chicken" = easter egg. hogSize(style) gives the
               footprint span (appended last per the migration note). Seeded from the ZONES registry
