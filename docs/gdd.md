...
- **Hog ball form (art only):** each common Hog style (`COMMON_HOG_STYLES`) has a defensive **ball** sprite — the hog curled spiky-side-out, painted by `hogBall` in `tools/art/hog.ts` and emitted as `HOG_BALL_ART` in `shared/sprite_art.ts`. It's one facing-independent pose (a ball reads the same from any side), so it lives outside the per-facing frame grid and isn't packed into the committed sheet. The art exists for the planned curl-when-defensive behaviour (curl on being picked up or attacked, then roll away); the runtime does not yet swap a Hog to it. Big Hogs (buff/dino) have no ball form. Preview with `pnpm art:preview --sheet=balls`.
