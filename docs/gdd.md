...
Dev mirrors prod: `pnpm dev` brings up local Postgres + Valkey (via `docker compose`) so persistence is always exercised the same way it runs in production. The server still tolerates a backend being absent — without `DATABASE_URL` / `REDIS_URL` the missing layer is skipped and state is in-memory only (invariant 6: it still boots) — but that is a fallback, not the default dev path.
