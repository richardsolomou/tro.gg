...
- Mascot integration: concept-inspired trogg/Hog sprite art has landed (`shared/sprites.ts` → `assets/sprites/`, rendered for troggs behind `avatar-sprites`). Further work is polish: richer poses, equipment overlays, and any artist-authored source pipeline if the project outgrows code-authored pixel art.
