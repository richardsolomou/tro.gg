...
- **Capacity cap before launch.** Connection and subscription limits are set so a viral night sheds or queues load instead of toppling the box; vertical scale (a bigger VPS) is the first lever. Per-zone entity caps (`MAX_HOGS_PER_ZONE`, `MAX_BOULDERS_PER_ZONE`) bound `/spawn` server-side, so no client can flood a zone and inflate every `wanderHogs` tick.
