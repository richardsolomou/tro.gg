...
- **Running:** holding **shift** while moving runs at `RUN_SPEED_TILES_PER_SEC` (7 tiles/sec *(initial)*) instead of walking, controlled by the optional `running` flag (off → shift is ignored, movement stays at walk speed). Run state rides the synced motion intent (`player.running`), so `projectMotion` derives the same faster position on every client — no per-frame sync, no determinism mismatch (invariants 2 & 3). Releasing shift, stopping, or a change of direction re-bases the origin at the current speed (the `move` reducer settles before storing the new intent). Troggs show a faster, hunched run clip (the run gait in `src/game3d/rig3d.ts`); Hogs always walk.
...
- **Creature rig (skeleton + clips).** Every creature is a hierarchy of named joint nodes — `Bob` (the gait dip), `LegL`/`LegR`, `Torso`, `ArmL`/`ArmR`, `Head`, plus the equip attach nodes `HandR` (main hand) and `HandL` (off hand) — built per species in `src/game3d/creatures3d.ts` over the shared machinery in `rig3d.ts`. Animation is **data played by the engine**: idle (breathing), walk, run (faster stride, forward hunch), and attack (wind-up → strike → recovery, timed to the synced equipment-use impulse) are procedurally authored `AnimationClip`s played through Three's `AnimationMixer` with crossfades. There are no per-facing frames — the model yaws smoothly toward its heading; the cardinal `Facing` survives as game logic (movement intents, interact targeting). Equipment use plays the attack clip, so the creature's **arm actually swings**; a held item is parented to the hand node, so it rides the swinging arm with no placement math. Taking damage plays a **hit flinch** — a short recoil opposite the facing plus a solid white flash (materials driven by `flash`), triggered when a creature's health drops; troggs and Hogs both flinch.
- **Held items** (pickaxe, shovel, sword, shield) render as models parented to the rig's hand nodes — a **main hand** (`HandR`) and an **off hand** (`HandL`), so combinations like sword + shield work. A new holdable is one small model builder in `src/game3d/items3d.ts` (grip at the origin, business end along +y), not a new character: parenting to the hand makes it swing through the walk/run cycle and lead the strike on attack for every species at once. Occlusion is free — the depth buffer replaces the 2D per-facing z-order and mirrored side art entirely. A per-item grip pitch tunes how each tool rests in the fist; the shield alone stays upright along the forearm. The inventory, equipped-slot, and spawn-button icons render from these same models (`src/game3d/icons3d.ts`), so every surface shows the exact object the world renders.
...
- What's equipped rides the zone's player sync, so others see what you're holding. Pressing `F` uses what the trogg is currently holding or wielding without stopping movement: a carried boulder or Hog is thrown, a pickaxe mines a faced boulder into Stone, a sword attacks a faced adjacent trogg, and a shovel currently shows its synced use animation only. The tools, the stone resource, and the boulder are low-poly props in the same code-built style as the creatures (`src/game3d/items3d.ts`); the world palette lives in `src/game3d/palette.ts`.
- **Model avatars:** a trogg renders as its jointed 3D model (styles `moss`/`stone`/`ridge` vary palette and brow), feet anchored at the centre of its tile. The per-trogg colour multiplies the body's materials — the 3D restatement of the sprite tint — so the same trogg is the same colour for everyone, every session; your own trogg gets a ground ring so you can pick it out. The old `avatar-sprites` flag (sprite vs placeholder marker) is retired with the 2D renderer. The dev `/preview` page inspects every creature model, clip, and held item, URL-addressable for shareable deep links; concept references live in `docs/art-refs/`.
- **Hog ball form:** each common Hog style has a defensive **ball** model — the hog curled spiky-side-out (`buildHogBall`, `src/game3d/creatures3d.ts`), one pose, no rig (a ball reads the same from any side). **A picked-up Hog renders as its ball** — the carry overlay swaps the upright model for the ball, so lifting a Hog curls it. The chicken easter egg has no ball and rides upright; big Hogs (buff/dino) aren't liftable, so they never need one. The further curl-when-attacked-then-roll-away behaviour is later work.
...
The target the rig is being built toward: a creature is not one model but a **stack of attachments** — base body, eyes, hair, outfit, armour pieces (breastplate, helmet, …), and held items — parented to its joints. Cosmetics and equipment are attachments you add, so players look different and gear is visibly worn. This is the paper-doll / layered-equipment pattern; the 3D renderer makes it dramatically cheaper than the 2D plan it replaces: an attachment is a model parented to a named joint, it animates with the body for free, and the depth buffer does the occlusion the 2D design needed per-facing layer tables and a composite-then-outline pass for.
...
Open questions to resolve before building: whether a slot carries a fit scale or just a transform; per-attachment tint vs separate models for colour variety; and how cosmetic-only attachments (hair, eyes) fold into the existing appearance/`restyle` model.
...
- Mascot integration: the trogg/Hog cast renders as code-built jointed 3D models (`src/game3d/creatures3d.ts`), with rig-driven limbs, the attack swing, and held main/off-hand items parented to the hands. HUD icons render from the same models. Further work is polish: richer clips, the attachment armour/cosmetic stack (see "Layered avatars and cross-species equipment"), and any artist-authored model pipeline (scripted Blender → glTF) if the project outgrows code-built geometry.
