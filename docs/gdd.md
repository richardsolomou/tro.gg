...
- Visible in-game HUD surfaces — chat history/input, account claim/rename controls, avatar colour swatches, and the top-left icon toolbar for Help, Inventory, and pre-alpha Commands — are HTML/CSS overlays above the Phaser canvas, so the browser owns layout, text input, focus, IME, and resize. The toolbar stacks icon-only toggles in normal layout instead of hard-coded offsets; labels live in accessible names/tooltips. HUD surfaces use `pointer-events` so a click on a panel is consumed by the DOM and a click on open space falls through to the canvas (click-to-move). World-space labels and speech bubbles over troggs stay in the Phaser scene. The Help panel and Commands panel list or expose only controls and commands whose feature flags are enabled this session, so they never advertise a disabled key or command.
