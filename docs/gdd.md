...
- **Server-owned, seeded from the registry.** Hogs have no identity. Their starting tiles come from the zone's `hogs` registry entry (`ZONES` in `shared`), seeded into the `hog` table on first connect (idempotent, like boulders), then moved only by `wanderHogs`. A Hog's `style` column is usually empty, meaning its skin is derived from id; Commands panel spawns store an explicit `HOG_STYLES` value so clicking a skin button creates that exact sprite.
...
- **Hog variation.** Ambient seeded Hogs usually leave `hog.style` empty, so each skin is derived from entity id (`HOG_STYLES`: `classic`, `snow`, `ember` — palette only, one shape), giving a zone a varied, stable crowd. Commands panel spawns store a concrete `hog.style` so the selected sprite is exact. Hogs are never tinted.
...
hog            id (PK, auto-inc), zoneId, x, y, dirX, dirY, movedAt, path, homeX, homeY, style
               ...
               a concrete HOG_STYLES id for exact Commands panel spawns. Seeded from the ZONES registry
...
ground_item    id (PK, auto-inc), zoneId, item, x, y
               ...
               a time from the Commands panel, and removed by `interact` when a trogg faces it and presses `E`.
...
Implemented world systems: a static shared `ZONES` registry, zone-scoped subscriptions, per-tile walkability, cardinal grid-locked WASD movement, boulder pushing, pick-up-and-carry interaction (`E`), starter tool pickups, inventory/equipment with `I`, equipped-item use with `F`, roaming Hogs, hold-shift-to-run, sprite avatars, trogg recolouring/restyling via Appearance (`P`), chat bubbles/history, a small synced ghost-trogg cosmetic (launch haunt + Commands panel summon), a help panel listing live controls, and a pre-alpha Commands panel for one-click spawn/reset/ghost tools. Some of these have optional client-side flag gates for remote rollout or kill-switch use; the current code-read flags are configured in PostHog and still have code fallbacks for local or unconfigured environments.
