...
- Mascot integration: the trogg concept art as the avatar spritesheet base. Initial programmer-art sprite sheets for troggs and Hogs exist (`public/sprites/`, generated by `tools/gen-sprites.ts` — the shared idle/walk rig per facing). Players now render with the trogg sheet behind the `avatar-sprites` flag (facing derived from motion intent; a colour aura preserves the placeholder's per-trogg identity), falling back to the colour marker when the flag is off or the sheets fail to load. Hog NPCs and held-item layers (M2) still to come.
