...
Current state this generalises from: the rig now resolves **both hands** as equip slots — `slotAnchor(kind, "mainHand" | "offHand", …)`, with per-slot z-order (the near/main hand in front except facing up, the far/off hand behind the body except facing down) — and `heldTransform` places any slot, so a **shield** renders in the off hand and a sword + shield compose correctly (verified in the preview and via `pnpm art:preview --sheet=held:sword+shield`). What's *not* wired yet is the gameplay side: only the **main hand** is equippable and synced (`equippedMainHand`) — off-hand equip/inventory/sync, and the multi-layer order *table* for worn armour, land with their consumers. The **trogg** is rig-driven (limbs from the rig); the **Hogs** are baked. Layers are then added cheapest-first (eyes/hair before armour).
...
```text
player         identity (PK), name, isGuest, zoneId, x, y, dirX, dirY, movedAt, online, lastChatAt, running, color, carrying, path, style, equippedMainHand, equipmentAction, equipmentActionAt, equippedMainHandInventoryId, faceX, faceY, carryingStyle, health, dead, respawnAt
...
- Mascot integration: concept-inspired trogg/Hog sprite art has landed (`shared/sprite_art.ts` + `shared/sprites.ts` → `assets/sprites/`, rendered for troggs behind `avatar-sprites`). Further work is polish: richer poses, equipment overlays, and any artist-authored source pipeline if the project outgrows indexed code-authored pixel art.
