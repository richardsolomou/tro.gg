...
The arc: wake sealed in a 5×4 birth cell (a hearth ember lights the room; the opening camera is raised and close so you see yourself enclosed), pick up the pickaxe (`E`), equip (`I`), mine the rubble corridor (`F`), cross the small glowmoss cavern in permanent cave-dark, and press **`E` beside the exit light** (a warm glow + the pickup-sparkle language). The `emerge` reducer verifies proximity server-side (invariant 3), **wipes the instance's rows** — it simply ceases — and transfers the trogg to `world` at `EMERGE_ARRIVAL`, a cave-mouth alcove carved into the south coast. The client reboots into the new zone behind the boot screen; the arrival opens on a raised reveal shot, fires `warren_emerged`, and plays the **first dawn** (~12s local-only blend from morning gold into the shared sky; the shared sky lock outranks it).
...
- **The instance is disposable and self-healing**: seeded on first connect (`seedBirthInstance`, idempotent), re-seeded if a returning mid-dig trogg's rows were somehow lost, wiped on emerge (`wipeBirthInstance`).
...
- Open threads: the emergence sightline (a landmark fire at town, walk-to-town breadcrumbs, naming at the fire), and polish on the dig length (4 rubble ≈ 30–45s).
