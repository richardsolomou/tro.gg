...
- **The world map** is a plus-shaped grid of **eleven procedurally generated zones** (`WORLD_GRID` in `shared/constants.ts`): `hog-town` at the centre and ten biomes around it — Glowvault and Starwell to the north, Mossglen and Boneyard to the south, Frosthollow/Shadowdeep/Floodways west, Rust Gallery/Emberrift/Dustworks east. Every zone is 64×44 *(initial)* with its own fixed seed. A **biome** picks the decoration mix in the shared generator (moss/gravel/glowmoss density, pool count) and the colour palette on the client (`BIOME_3D`, hue-shift transforms of the cave palette with accent overrides) — colours never touch the shared data, so the module stays colour-blind.
- **Edge gates and travel:** grid neighbours open reciprocal gates mid-edge — the generator carves the rim open and tunnels inward until it meets the cave (after the connectivity fill, so a gate can't be sealed off; nothing seeds inside a gate tunnel). Gates render as flanking pillars with a lantern glow. Standing on or beside a gate, **interact (`E`) travels**: the server re-derives the position, moves the player row to the target zone just inside the reciprocal gate (settled, idle), and lazily seeds that zone's boulders/Hogs/items on first entry (`zone_traveled` event, docs/analytics.md). The client then follows the reconnect pattern — remember the zone in `localStorage`, fade to black, and reload; every table re-derives from subscriptions on boot, so travel swaps zone, terrain, and rows without hand-rewiring anything. The world subscription also carries the self row from any zone, so a stale local record self-corrects with one reload. `assertZones` validates gate walkability and reciprocity for every pair.
