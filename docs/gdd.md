...
- **WASD / arrow keys:** hold to move, release to stop. **Free 8-directional movement:** held keys combine, so W+D walks the diagonal at the same overall speed (the shared projection normalises diagonal velocity); within one axis the newest held key wins. A press moves (and faces) immediately — no turn-in-place beat — and a trogg can rest anywhere, not just on tile centres. Axis values are coerced to unit steps server-side, never trusted (invariant 3). WASD directions are mapped through the camera heading (snapped to the nearest quadrant), so after orbiting the camera, "up" is still screen-up.
...
- **Facing stays cardinal:** the synced standing facing (`player.faceX`/`faceY`) is the intent's dominant axis, so the tile mechanics that hang off facing — interact, attack, push — keep their crisp grid targeting on top of free movement.
...
Implemented world systems: a static shared `ZONES` registry, zone-scoped subscriptions, per-tile walkability, free 8-directional WASD movement with sliding collision, boulder pushing, pick-up-and-carry interaction (`E`), throwable boulders/Hogs, starter tool pickups, inventory/equipment with `I`, equipped-item use with `F`, sword attacks, trogg and Hog health, death drops, timed respawn, roaming Hogs, hold-shift-to-run, 3D avatars, trogg recolouring/restyling via Appearance (`P`), chat bubbles/history, a small synced ghost-trogg cosmetic (Commands panel summon), a help panel listing live controls, and a pre-alpha Commands panel for one-click spawn/reset/ghost tools. Some of these have optional client-side flag gates for remote rollout or kill-switch use; the current code-read flags are configured in PostHog and still have code fallbacks for local or unconfigured environments.
