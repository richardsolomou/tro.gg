...
The arc: wake in the **middle of the glowmoss cavern** (a hearth ember lights the wake-up spot; the opening camera is raised and close), pick up the pickaxe beside you (`E`), equip (`I`), mine the **two rubble rocks** plugging the neck up to the mouth (`F`), and **walk out — the walk is the door**. Through the mouth you see the actual outside: the client renders a fog-free, daylight-lit window of the world map (the ~10 rows of coast in front of the arrival alcove, on its own light layer, aligned tile-for-tile), so emerging reads as walking outdoors, not switching instances. No props, no sparkle — an entrance. The `emerge` reducer verifies proximity server-side (invariant 3), **wipes the instance's rows** — it simply ceases — and transfers the trogg to `world` at `EMERGE_ARRIVAL`, a cave-mouth alcove carved into the south coast. The client reboots into the new zone behind the boot screen; the arrival opens on a raised reveal shot, fires `warren_emerged`, and plays the **first dawn** (~12s local-only blend from morning gold into the shared sky; the shared sky lock outranks it).
