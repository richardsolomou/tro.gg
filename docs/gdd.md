...
- **First person is the default camera**: the world opens at eye level (pointer-lock mouse-look — click the world to grab the cursor; you can look up at the sky), your own model leaves the frame, and WASD stays camera-relative. `V` swaps to the overhead orbit camera and back.
...
- Chat history and the typing field are HTML overlays (a real `<input>`); speech bubbles over troggs render as sprites in the 3D world. Chat content is added as a DOM text node, never as HTML markup, so a message can't inject markup.
