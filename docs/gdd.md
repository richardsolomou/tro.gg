...
- **Sword attacks:** pressing `F` with a sword equipped swings along the trogg's exact aim vector and damages the nearest online, living trogg — a trogg outranks a Hog at equal distance — whose **hit circle** is in reach: within `MELEE_RANGE_TILES` (1.7 *(initial)*, centre to circle edge) and inside the ±55° swing cone (`MELEE_ARC_RAD`; point-blank inside `MELEE_POINT_BLANK_TILES` waives the angle, and a target's radius widens the cone by what its silhouette subtends — `shared/combat.ts`). Troggs have a 0.45-tile radius, Hogs 0.42 scaled by `hogSize`, so a giant is as hittable as it looks. The client passes its aim, but the server re-derives every position with the same projections the client renders (invariant 3).
