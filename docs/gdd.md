...
- Visible in-game HUD surfaces — chat history/input, the top-right account claim control, and the top-left toggle bar (Help, plus the Appearance panel with rename/colour/style) — are HTML/CSS overlays above the Phaser canvas, so the browser owns layout, text input, focus, IME, and resize. They use `pointer-events` so a click on a panel is consumed by the DOM and a click on open space falls through to the canvas (click-to-move). World-space labels and speech bubbles over troggs stay in the Phaser scene. The help panel lists only the controls and commands whose feature flags are enabled this session, so it never advertises a disabled key or command.
