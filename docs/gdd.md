...
- **The world is ONE seamless zone** (slug `world`): a 192×220 tile coordinate space laid out as a plus-shaped grid of **eleven biome regions** (`WORLD_REGIONS` in `shared/worldgen.ts`) — Hog Town at the centre; Glowvault and Starwell north; Mossglen and Boneyard south; Frosthollow, Shadowdeep, Floodways west; Rust Gallery, Emberrift, Dustworks east. Regions are *character, not instances*: natural carved passages join neighbours and you simply walk across. A **biome** picks the decoration mix in the generator and the colour palette on the client (`BIOME_3D`); tinting is applied per tile, so region borders are exact.
...
- **The client streams the world in proximity chunks** (`src/game/terrain.ts`): 32-tile chunks build nearest-first (throttled per frame) within a radius that scales with camera zoom, and dispose with hysteresis when out of range — Google-Maps style. Each chunk is one floor texture (tiles blitted from the biome patch painters), one instanced wall mesh (per-instance biome colours), and at most two glowmoss lights. Fog opens with the zoom, distant creatures/boulders/items hide beyond it (`cullDistant`), and the shadowing key light rides the camera focus. Zoomed out, the whole plus renders as visible biome fields.
