...
- Visible in-game HUD surfaces — chat history/input, the top-right account claim control, and the top-left icon toggle bar (Help `?`/`H`, Settings `O`, Appearance `P`, Inventory `I`, and the pre-alpha Commands panel `` ` ``) — are HTML/CSS overlays above the canvas, so the browser owns layout, text input, focus, IME, and resize. They use `pointer-events` so a click on a panel is consumed by the DOM and a click on open space falls through to the canvas (click-to-move). The toggle bar is an accordion: opening one menu closes the others, and `Esc` closes the open menu. World-space labels, health bars, and speech bubbles over troggs are camera-facing canvas sprites inside the 3D scene (`src/game/overlays.ts`), drawn depth-test-off so a wall never swallows a nameplate. Help lists only enabled controls, and the Commands panel exposes only enabled debug tools.
