...
- Mascot integration: the trogg concept art as the avatar spritesheet base. Initial programmer-art sprite sheets for troggs and Hogs exist (`public/sprites/`, generated by `tools/gen-sprites.ts` — the shared idle/walk rig per facing) but aren't wired into the renderer yet; placeholder markers still stand.
