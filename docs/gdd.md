...
- **Every weapon has its own damage** (`WEAPON_DAMAGE` *(initial)*: sword 25, axe 18, pickaxe 15, shovel 12). Gathering tools resolve their node first — a pickaxe swing that reaches a boulder mines it and spares the Hog behind it; an axe fells the tree — and wound creatures only when no node claims the swing. Off-hand items (shield, torch) and bare fists deal nothing.
