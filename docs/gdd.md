...
- **A push is a deliberate move *into* a boulder, not arriving beside one.** Walking up to a boulder stops the trogg flush against it like a wall (it's an occupied tile). From that flush position, pressing into the boulder — a fresh movement intent along the facing direction — shoves it one tile, if the tile beyond is open floor (no wall, no other boulder). The trogg must be **tile-aligned and flush**. Just arriving next to a boulder while walking never pushes it, so you can stand beside one without shoving it; the shove takes a separate press in.
- **No tick** (invariant 1) and **server-authoritative** (invariant 3). The client fires `push` off the *movement intent* it sends when grid-locked motion flushes (a motion transition — never per frame, invariant 2): if that intent faces a boulder the trogg is squarely on a centre against, it calls the `push` reducer. The server re-derives the trogg's position from its stored intent, validates alignment + a clear destination, moves the boulder one tile, and re-bases the trogg's motion to the flush tile.
- **One shove per press.** Re-basing leaves the boulder one tile ahead, and the trogg follows into the vacated tile and stops flush again — so a held key shoves once, and moving the boulder across a room is one press per tile, never a continuous slide. Spamming `push` can't help: the boulder isn't faced again until the trogg catches up.
...
- **Sprite avatars (behind `avatar-sprites`):** a trogg renders as the layered avatar sprite — programmer pixel art generated from `shared/sprites.ts` (4 facings × idle/walk, troggs and Hogs sharing one rig), feet anchored at the centre of the tile (not its bottom edge, so a grid-locked trogg stands in the middle of its tile). The stable per-trogg colour now rides as a sprite **tint** (a deterministic projection of its durable id — derived, never stored, like a level from XP), so the same trogg is the same colour for everyone, every session; your own trogg gets a ground ring so you can pick it out. The committed sprite sheet asset (`assets/sprites/`) is the reviewable export; the client paints the same art into a texture at runtime.
...
Boulder pushing landed in M0 too, also at maintainer direction (see [Pushing](#pushing)) — pushable boulders as dynamic obstacles, shoved one tile per deliberate press into them, behind the `boulder-pushing` flag. It reuses the walkability collision (a boulder is just an occupied tile) and the intent model (the push fires off the movement intent and re-bases motion, no tick), so it's an extension of movement rather than new infrastructure.
