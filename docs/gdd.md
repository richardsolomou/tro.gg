...
- **Nodes have health and take swings** (`BOULDER_MAX_HEALTH` 45, `TREE_MAX_HEALTH` 54 *(initial)* — roughly three average hits each). Every tool swing that reaches its node rolls the tool's `WEAPON_DAMAGE` range into the node's health, and the client answers each hit with the same feedback a creature gives: a white hit flash and a floating damage number. The breaking hit deletes the row and grants the resource — whatever weapon dealt it.
...
- **Mine a boulder with a pickaxe.** Pressing `F` with a pickaxe equipped strikes the nearest boulder in melee reach along the aim (the same reach + swing-cone geometry as combat); the breaking hit lands one **Stone** in inventory. No free slot for a new stone stack → the breaking hit is refused and the boulder stays, barely standing.
- **Fell a tree with an axe.** The exact mirror: `F` with an axe equipped chops the nearest tree in reach; the felling hit grants one **Wood**. The axe shares the pickaxe's overhead `chop` wield class.
...
- **Ground items go into inventory.** Empty-handed, pressing `E` beside a ground item removes that `ground_item` row and adds the item to the trogg's inventory. Starter pickups in `hog-town` are a pickaxe, shovel, axe, sword, and shield. Items are not solid; pickup uses the same adjacent scan and faced-tile priority as Hog pickup.
...
- **Hog health:** a roaming Hog carries `hogMaxHealth(style)` health on its row — `HOG_MAX_HEALTH` (60 *(initial)*) for a common, scaled by footprint area for giants (a 2×2 is four commons' worth). Damaged Hogs show a small health bar. At zero health the Hog becomes a **corpse**: settled where it fell, stopped, lying flat in the dead stance — not solid, not targetable, not liftable — until the regen sweep reaps it after `NPC_CORPSE_MS` (30s *(initial)*).
