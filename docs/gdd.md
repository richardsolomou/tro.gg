...
- The current world ships a single shared zone *(working slug `hog-town`, 40×24 (initial))*, laid out as a plaza of courts (see [Courts and play props](#courts-and-play-props)); additional zones, starting areas, and gates can be added when they help the current gameplay. When editing a shipped tilemap, never place a new wall on a tile that shipped as floor — seeded rows in live databases keep their coordinates across deploys, and a redeploy must not bury them (the connect handler nudges a trogg out of a wall, but boulders, Hogs, and ground items have no such rescue).
...
### Courts and play props

The community mini-game surface: the shared zone is laid out as a **plaza of courts** — regions of the tilemap dressed and seeded to *suggest* a game without enforcing one. Players supply the rules by convention (announced in chat); the world supplies props and a legible place to play. Because the whole zone is one subscription and one screen, every court is visible from every other — you see a game being played across the plaza and walk over to join it, no room switch.

- **A court is design data, not a mechanic.** It's a region of the zone's tilemap (floor dressing + wall shapes) plus prop seeds in the registry — no schema, no reducers, no court table. The current plaza has a **curling lane** in the north-east (a gravel lane; push or throw staged boulders east onto the target plates), a **herding pen** in the south-east meadow (a walled pen with a one-tile gate; carry or throw Hogs in and they light the plates they sit on), a **boulder garden** in the south (a 2×2 plate square with loose boulders — Sokoban by convention), and the open **commons** around the spawn.
- **Props work anywhere; courts only concentrate them.** A boulder pushes and throws the same on any floor, a plate lights wherever the tilemap puts one. Courts are suggestions — dragging the props into the commons and inventing a different game is the desired outcome, not a violation.
- **The pressure plate (`o`)** is the first play prop: a stone plate tile that **lights up while a trogg, Hog, or boulder rests on it**. The lit state is derived client-side each frame from rows every client already syncs (the same projected tiles collision uses), so every player sees the same plate light with zero server state, no reducer, and no new sync — it stays in the cosmetic glyph class. Targets, finish lines, and puzzle goals become legible without the server knowing a game exists. Server-side rules that *react* to plates (doors, scores) are explicitly later work and would move plates out of the cosmetic class when they land.
- **No scores, no rounds, no enforcement.** Low stakes is the point: nothing is tracked, so nothing is lost. If a game wants stakes, the players keep score in chat. Per-court rule overrides (e.g. no death-drop inside a court) and prop mechanics that deepen play (a rolling Hog ball) are separate later work.
...
Implemented world systems: a static shared `ZONES` registry, zone-scoped subscriptions, per-tile walkability, cardinal grid-locked WASD movement, boulder pushing, pick-up-and-carry interaction (`E`), throwable boulders/Hogs, starter tool pickups, inventory/equipment with `I`, equipped-item use with `F`, sword attacks, trogg and Hog health, death drops, timed respawn, roaming Hogs, hold-shift-to-run, sprite avatars, trogg recolouring/restyling via Appearance (`P`), chat bubbles/history, a small synced ghost-trogg cosmetic (Commands panel summon), a help panel listing live controls, a pre-alpha Commands panel for one-click spawn/reset/ghost tools, and the plaza-of-courts layout with client-derived pressure plates (GDD "Courts and play props"). Some of these have optional client-side flag gates for remote rollout or kill-switch use; the current code-read flags are configured in PostHog and still have code fallbacks for local or unconfigured environments.
