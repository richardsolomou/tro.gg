...
- **Facing stays cardinal:** the synced standing facing (`player.faceX`/`faceY`) is the heading's dominant axis, so the tile mechanics that hang off facing — interact, attack, push — keep their crisp grid targeting on top of free movement. `facingTile` treats a *mostly-cardinal* press (minor axis ≤ 0.35 of the major) as facing its dominant neighbour, so a slightly rotated camera doesn't break pushing.
...
- **A push is walking *into* a boulder, not arriving beside one.** Hold a cardinal into a boulder you're lined up with and it slides ahead of you, one tile at a time, for as long as you keep pushing — like walking. Releasing as you reach it stops the trogg flush against it like a wall (it's an occupied tile), no shove. The trogg must be **flush and lined up within the `facingTile` tolerance** (walking into a boulder clamps exactly flush, so a deliberate press always qualifies; a diagonal press never shoves), and the tile beyond the boulder must be open floor (no wall, no other boulder, no Hog). The accepted shove re-bases the trogg onto the whole tile, squarely in the push lane.
...
- **Pre-alpha Commands panel:** debug tools live in the top-left Commands panel — including a client-side **Hitboxes** toggle (`B`) that shows every creature's and boulder's combat hit circle plus the local trogg's melee reach ring and swing cone (the cone follows the live aim and brightens while a use is in flight), not chat slash commands. The spawn grid uses icons rendered from the actual world models: one boulder button, one button per `HOG_STYLES` skin/size, and one button per `SPAWNABLE_ITEM_IDS` equipment item (`pickaxe`, `shovel`, `sword`, `shield`). `stone` remains a valid inventory/resource item, but it is gathered through mining and is not exposed or accepted by debug spawning. Each click requests exactly one spawn; if someone wants ten Hogs, they press a Hog button ten times. The `spawn` reducer places the thing on a nearby free tile around the caller, starting with the tile they face, and clamps inserts to `MAX_BOULDERS_PER_ZONE`, `MAX_HOGS_PER_ZONE`, `MAX_GROUND_ITEMS_PER_ZONE`, and available floor so the UI can be abused without bypassing caps.
...
Implemented world systems: a static shared `ZONES` registry, zone-scoped subscriptions, per-tile walkability, free camera-relative WASD movement with sliding collision, boulder pushing, pick-up-and-carry interaction (`E`), throwable boulders/Hogs, starter tool pickups, inventory/equipment with `I`, equipped-item use with `F`, sword attacks, trogg and Hog health, death drops, timed respawn, roaming Hogs, hold-shift-to-run, 3D avatars, trogg recolouring/restyling via Appearance (`P`), chat bubbles/history, a small synced ghost-trogg cosmetic (Commands panel summon), a help panel listing live controls, and a pre-alpha Commands panel for one-click spawn/reset/ghost tools. Some of these have optional client-side flag gates for remote rollout or kill-switch use; the current code-read flags are configured in PostHog and still have code fallbacks for local or unconfigured environments.
