...
- **Hog variation.** Ambient seeded Hogs usually leave `hog.style` empty, so each skin/size is derived from entity id (`HOG_STYLES`: `classic`, `classic-big`, `snow`, `snow-big`, `ember`, `ember-big`), giving a zone a varied, stable crowd. Commands panel spawns store a concrete `hog.style` so the selected sprite is exact. Big Hogs are visually larger but still occupy one tile. Hogs are never tinted.
...
hog            id (PK, auto-inc), zoneId, x, y, dirX, dirY, movedAt, path, homeX, homeY, style
               ...
               a concrete HOG_STYLES skin/size id for exact Commands panel spawns. Seeded from the ZONES registry
