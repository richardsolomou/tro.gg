...
- **Held items** (pickaxe, shovel, sword, shield) render as models parented to the rig's hand nodes — a **main hand** (`HandR`) and an **off hand** (`HandL`), so combinations like sword + shield work. A new holdable is one small model builder in `src/game3d/items3d.ts` (grip at the origin, business end along +y), not a new character: parenting to the hand makes it swing through the walk/run cycle and lead the strike on attack for every species at once. Occlusion is free — the depth buffer replaces the 2D per-facing z-order and mirrored side art entirely. A per-item grip pitch tunes how each tool rests in the fist; the shield alone stays upright along the forearm. The inventory panel still renders the pixel `ITEM_ART` icons (`shared/item_art.ts`) — icons are 2D UI, not world objects.
...
- Mascot integration: the trogg/Hog cast renders as code-built jointed 3D models (`src/game3d/creatures3d.ts`), with rig-driven limbs, the attack swing, and held main/off-hand items parented to the hands. The concept-inspired pixel sprite art (`shared/sprite_art.ts` → `assets/sprites/`) remains the art-direction reference and the inventory icon source. Further work is polish: richer clips, the attachment armour/cosmetic stack (see "Layered avatars and cross-species equipment"), and any artist-authored model pipeline (scripted Blender → glTF) if the project outgrows code-built geometry.
