...
Brazier upkeep/gutter/relight, a trogg going ember/dormant, and an ignition project resolving (succeeding or failing) are all live but emit no dedicated event — each runs inside a scheduled reducer (`brazierUpkeep`, `wanderPresence`), not a procedure, and only procedures have an analytics capture step in this codebase (see the procedure-wrapper paragraph below); these stay visible to clients via row diffs on the synced `brazier`/`player`/`project` tables instead. `project_contributed` fires when an ignition starts (`project: "ignition", item: "ember_heart", qty: 1`); `project_completed` is reserved for a future project type whose completion is worth a dedicated event, or for wiring ignition's resolution through a procedure later. Register any new event here in the same change that adds it, per the rule below.
