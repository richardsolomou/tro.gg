...
New events anticipated by the fire-and-dark design (a brazier igniting or gutter, presence transitions) are open — brazier upkeep/gutter and a trogg going ember/dormant are both live but don't emit dedicated events yet (they're visible via row diffs on the synced `brazier`/`player` tables); the `ignition`/`project_contributed`/`project_completed` events above cover ignition once brazier sites use them (see gdd.md Roadmap). Register any new event here in the same change that adds it, per the rule below.
