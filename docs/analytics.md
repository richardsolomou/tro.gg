...
| `project_contributed` | `project, item, qty` | Igniting a brazier spends its stake from the stockpile — fired by the `interact` procedure the moment a carried ember-heart is delivered on qualifying ground (GDD "The fire and the dark" → Ignition) |
| `project_completed` | `project, source` | An ignition's hold-the-point window elapses and its brazier lights. Captured client-side from the authoritative `project` row transitioning to `succeeded`, the same pattern as `player_respawned`, because it's driven by a scheduled reducer, not a procedure call |

Brazier gutter/relight and a trogg's bright/ember/dormant transitions are not tracked as events — both are continuously time-derived (GDD "The fire and the dark" → Presence, Territory and permanence) rather than discrete row transitions a procedure or a client observer can cleanly capture once. Add events for them if a concrete product need shows up.
...
| `dark-creature-reset` | Commands panel dark-creature population reset control | On |
...
Retired by the 3D renderer port: `avatar-sprites` (trogg sprite avatars vs the placeholder colour marker) is no longer read — the 3D client always renders models. The PostHog flag stays live while the 2D client is still the deployed production build; archive it in project 314596 when this port ships. Retired with boulder pushing: `boulder-pushing` is no longer read — boulders are mining nodes, not pushable; archive alongside `avatar-sprites`. Retired with the fire-and-dark pivot: `roaming-hogs` and `hog-reset` are no longer read — the roaming-Hog mechanic and its reset tool are gone, replaced by dark creatures and `dark-creature-reset`; both are archived in project 314596.

Dark-creature rendering and subscription are not behind a flag: pre-alpha, no live userbase yet to roll the mechanic out to gradually (design pillar 5 — add a flag only when remote rollout, a kill-switch, or an experiment is actually useful).

PostHog project audit (2026-07-05): all code-read flags above are configured in PostHog project 314596 and active. They are intentionally still in use because they cover remote rollback, production-only debug command governance, or visible UI capabilities that should not advertise disabled controls. `interact` was created 2026-06-25 with the carry mechanic; `dark-creature-reset` was created with the dark-creature reset tool; `trogg-restyle` was created 2026-06-27 with avatar body styles. Planned future flags should be added here, and created in PostHog, when code starts reading them.
