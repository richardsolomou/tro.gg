...
| `object_thrown` | `zone, kind, range, source?, hit_target?` | Player throws what they carry; `hit_target` is `trogg` or `dark_creature` when the throw damages a character. Dormant for the same reason as `object_dropped` above |
| `region_claimed` | `zone, region, stone_cost` | Player claims a cleared penumbra region via empty-handed `interact` — every living dark creature in it is dead and the stockpile covered `stone_cost` stone, so a brazier goes down and the region turns interior (see gdd.md "The fire and the dark" → Territory claiming) |
...
| `level_up` | `zone, skill, level, source?` | An active trogg's XP grant crossed a level boundary (`useEquipped` procedure — gather breaking hits and combat damage). `skill` is `mining`, `woodcutting`, or `combat`; `level` is the level just reached |
| `item_crafted` | `zone, item, stone_cost, wood_cost, skill, level, source?` | Player crafts a recipe at the First Fire's station (`craftItem` procedure) — the attribution record for stockpile spending (gdd.md "Crafting") |
...
Planned events, not yet emitted by any code: `zone_entered`, `action_started`, `resource_gathered`, and `xp_gained` (deliberately unshipped for now — one event per swing is too hot; `level_up` covers the milestone) — move each into the table above when the emitting code ships. Brazier upkeep/gutter and a trogg going AFK are anticipated by the fire-and-dark design but emit no dedicated event — each runs inside a scheduled reducer, not a procedure, and only procedures have an analytics capture step in this codebase (see the procedure-wrapper paragraph below); these stay visible to clients via row diffs on the synced `brazier`/`player` tables instead. Register any new event here in the same change that adds it, per the rule below.
...
PostHog project audit (2026-06-27): the code-read flags at that time were configured in PostHog project 314596 and active. `dark-creature-rendering` is newer than the audit; verify it exists in project 314596 (create at 100% on if missing — its client fallback is on). `boulder-reset` and `dark-creature-reset` are retired: the Commands panel reset buttons were removed (2026-07-07) and no code reads them — the flags can be deleted in PostHog. Flags are intentionally kept live because they cover remote rollback, production-only debug command governance, or visible UI capabilities that should not advertise disabled controls. Planned future flags should be added here, and created in PostHog, when code starts reading them.
