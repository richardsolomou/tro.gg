...
test("mining takes swings: each hit chips the boulder, the breaking hit deposits stone into the stockpile", () => {
  ...
  // the yield deposits straight into the shared stockpile — never a ground item or personal inventory
...
test("an axe chips a fresh tree, and the breaking blow on a worn one deposits wood into the stockpile", () => {
  ...
  assert.equal(ctx.db.groundItem.rows().some((r: any) => r.item === "wood"), false);
...
test("an axe only scratches a boulder — the wrong tool works at a fraction", () => {
  ...
  assert.equal(ctx.db.stockpile.item.find("stone"), undefined); // not broken yet — nothing deposited
  assert.equal(ctx.db.inventory.rows().some((r: any) => r.item === "stone" || r.item === "wood"), false);
});

test("gathering never checks personal inventory — a full inventory still mines and deposits", () => {
  ...
  assert.equal(ctx.db.stockpile.item.find("stone")?.qty, 1); // deposited regardless of inventory fullness
});

test("stockpile deposits are capped — a full stockpile accepts no more", () => {
  const { ctx, me } = withPlayer({ x: 69, y: 96, equippedMainHand: "pickaxe" });
  const pickaxe = ctx.db.inventory.insert({ id: 0n, playerId: me, item: "pickaxe", qty: 1 });
  ctx.db.player.identity.update({ ...ctx.db.player.identity.find(me), equippedMainHandInventoryId: pickaxe.id });
  ...
  assert.equal(ctx.db.boulder.rows().length, 0); // the swing still breaks the node
  assert.equal(ctx.db.stockpile.item.find("stone")?.qty, STOCKPILE_CAP); // but the deposit is wasted
});
...
test("interact deposits a stockpile-item ground pickup straight into the stockpile", () => {
  const { ctx, me } = withPlayer({ x: 69, y: 96, carrying: "" });
  ctx.db.groundItem.insert({ id: 0n, zoneId: ZONE, item: "stone", x: 70, y: 96, qty: 3 });
  interact(ctx, { dirX: 1, dirY: 0 });
  assert.equal(ctx.db.groundItem.rows().length, 0);
  assert.equal(ctx.db.stockpile.item.find("stone")?.qty, 3);
  assert.equal(ctx.db.inventory.playerId.filter(me).length, 0);
});
