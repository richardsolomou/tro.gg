...
| ---- | ---------- | ------- |
| `src/` | Phaser 4 + Vite game client (`spacetimedb` SDK, `posthog-js`), grouped into `game/` (scenes + world rendering), `ui/` (HTML/CSS HUD), and `net/` (connection + the generated `module_bindings/`, produced from the module schema) | Cloudflare Workers |
...
| `instrumentation/` | Trusted PostHog Node sidecar. It polls the SpacetimeDB HTTP SQL API for backend-derived events and emits sidecar logs/exceptions without creating a live game connection. | Node process on the backend host |
...
pnpm sidecar           # optional: backend table telemetry + PostHog Logs sidecar
```
...
`just build` builds the client; `just typecheck` checks the client, sidecar, and module; `just test` runs the unit tests — shared pure logic (`shared/`), the client prediction controller (`src/movement.test.ts`), and the server reducers (`test/`, which call the real reducers against an in-memory `ctx` since the module can't load under node).
