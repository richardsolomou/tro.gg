...
- **Client → Cloudflare Workers (static assets).** An assets-only Worker configured by [`wrangler.jsonc`](wrangler.jsonc) — `assets.directory` is `client/dist`. The build emits two pages: the landing at `/` and the game at `/play`. Build command `pnpm build:client`, deploy command `npx wrangler deploy`. Set `VITE_COLYSEUS_URL` (the server's `wss://` URL), `VITE_POSTHOG_KEY`, and `VITE_POSTHOG_HOST` as **build** environment variables (baked into the bundle at build time, not read at runtime), along with `NODE_VERSION=24`.
