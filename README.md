...
- **Server → Hetzner VPS.** `pnpm --filter @trogg/server build && pnpm --filter @trogg/server start`. Set `PORT`, `CLIENT_ORIGIN` (the Pages origin), `DATABASE_URL` (Postgres), `REDIS_URL` (cache + Colyseus presence/driver), and `AUTH_SECRET` (a stable random string signing guest credentials) in the environment.
