...
/** How many troggs render at once: the nearest N within
...
export class World3D {
   ...
   *  Only the nearest `CREATURE_BUDGET` render; the rest hide until
  ...
  private readonly tick = () => {
    ...
    for (const entry of this.tracked.values()) {
      ...
      else if (!entry.player.dead) this.crowd.add(motion.x, motion.y, Math.atan2(motion.dirX, motion.dirY), entry.style, entry.baseColor);
