...
import { DEEP_WATER_TILE, GLOWMOSS_TILE, GRAVEL_TILE, isWalkable, MOSS_TILE, regionAt, WALL_TILE, WATER_TILE, type Zone } from "@trogg/shared";
...
const PATCH = 4;
/**
 * Rock height is terrain-shaped, not dice-rolled: a distance field from the
 * walkable floor keeps the rock beside paths at shoulder height and raises the
 * cores of formations into mountains, while low-frequency noise makes ranges
 * undulate and throws occasional peaks. Deterministic, so every client carves
 * the same skyline.
 */
const ROCK_EDGE = 1.3;
...
function rangeNoise(x: number, y: number, scale: number, salt: number): number {
  const gx = x / scale;
  const gy = y / scale;
...
export function buildTerrain(zone: Zone): Terrain3D {
  ...
  const rockDepth = rockDepthField(zone);
  const rockHeight = (x: number, y: number): number => {
    const depth = Math.min(8, rockDepth[y * zone.width + x] ?? 1);
