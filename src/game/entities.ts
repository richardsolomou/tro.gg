...
export function createEntities(scene: THREE.Scene) {
   ...
   *  collision state changes under a live intent (an obstacle claims the tile ahead and the
  ...
  /** Point a creature at its motion and pick the gait action. */
  const driveCreature = (
    model: CreatureModel,
  ...
  /** Sync the carried overlay to the player row. */
  const applyCarry = (entry: Tracked) => {
    const kind = entry.player.carrying;
    const style = entry.player.carryingStyle;
