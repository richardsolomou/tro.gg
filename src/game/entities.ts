...
export function createEntities(scene: THREE.Scene) {
   ...
   *  collision state changes under a live intent (a moving obstacle claims the tile
   *  ahead and the clamp rewinds the projection; an authority snap lands) — instead
   *  of popping, the rendered marker absorbs the jump into an offset that glides out
   *  over ~120 ms.
