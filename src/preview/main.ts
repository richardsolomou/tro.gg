...
import { TROGG_STYLES, wieldOf, type Kind } from "@trogg/shared";
 ...
 * `/preview?view=holder&creature=trogg:ridge&item=sword&off=shield&mode=attack&paused=1&scrub=0.35&bones=1&yaw=-1.2`.
...
const CREATURES: CreatureChoice[] = TROGG_STYLES.map((style) => ({ kind: "trogg" as Kind, style }));
...
function rebuild(): void {
  ...
  // The in-game dead stance, verbatim: flat on the ground, faded. Frozen — the
  // tick skips the mixer.
  if (state.mode === "dead") {
...
function mountControls(): void {
  ...
  for (const c of CREATURES) {
    ...
    creatures.appendChild(slot(troggIcon(c.style), () => state.creature.kind === c.kind && state.creature.style === c.style, pick, true));
  }
  ...
  // rig; the hit flinch runs on its own cycle, so pause/scrub only apply to the
  // clip modes.
