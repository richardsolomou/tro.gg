...
function harness(over: Partial<SelfControllerDeps> = {}) {
  ...
  const self = createSelfController({
    conn,
    // Like production, the bounds treat obstacle and boulder tiles as unwalkable, so the
...
test("waiting against a blocking obstacle sends one standing face update", () => {
  const h = harness();
...
test("walking flush into an obstacle stops the trogg, keeping it idle (no stale intent)", () => {
  const h = harness();
  h.self.onIntent(right, true);
  h.moves.length = 0;
  h.obstacleTiles.add("1,0"); // an obstacle on the tile directly ahead
  h.self.update(h.entry, h.motion(0, 0, 1, 0), 0); // flush on centre against the obstacle
...
test("WASD resumes the held direction once a blocking obstacle moves off", () => {
  ...
  h.obstacleTiles.add("1,0"); // an obstacle flush on the tile ahead
  ...
  assert.equal(h.moves.length, 0);
  h.obstacleTiles.clear(); // the obstacle moves off
