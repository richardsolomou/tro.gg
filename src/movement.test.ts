...
function harness(over: Partial<SelfControllerDeps> = {}) {
  ...
  const self = createSelfController({
    conn,
    // Like production, the bounds treat creature and boulder tiles as unwalkable, so the
    // controller's can-I-progress probe collides with them (world3d's zoneBounds wiring).
    bounds: { width: 20, height: 20, isWalkable: (x, y) => !creatureTiles.has(`${x},${y}`) && !boulderTiles.has(`${x},${y}`) } as ZoneBounds,
    getSelf: () => entry,
  ...
  return { self, entry, player, moves, faces, moveTos, creatureTiles, boulderTiles, destinations, motion };
}
...
test("waiting against a blocking creature sends one standing face update", () => {
  const h = harness();
  h.creatureTiles.add("1,0");
  h.self.onIntent(right);
...
test("walking flush into a creature stops the trogg, keeping it idle (no stale intent)", () => {
  const h = harness();
  h.self.onIntent(right, true);
  h.moves.length = 0;
  h.creatureTiles.add("1,0"); // creature on the tile directly ahead
  h.self.update(h.entry, h.motion(0, 0, 1, 0), 0); // flush on centre against the creature
...
test("WASD resumes the held direction once a blocking creature moves off", () => {
  const h = harness();
  h.self.onIntent(right, true); // walking right
  h.creatureTiles.add("1,0"); // creature flush on the tile ahead
  ...
  assert.equal(h.moves.length, 0);
  h.creatureTiles.clear(); // creature ambles off
  h.self.update(h.entry, h.motion(0, 0, 0, 0), 32); // tile clear → resume walking
  assert.deepEqual(h.moves.at(-1), right);
});

test("a click-to-move route stalled on a creature re-issues toward the clicked tile", () => {
  const h = harness();
  h.self.onClick({ x: 5, y: 0 });
  h.self.update(h.entry, h.motion(0, 0, 0, 0, true), 0); // flush fires the initial moveTo
  h.moveTos.length = 0;
  // Server routed us partway, then a creature sealed the next tile: path set, no heading, not arrived.
