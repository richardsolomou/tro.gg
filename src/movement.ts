 ...
 *  exists yet, so re-routing around an obstacle — or waiting for one to clear the only
 *  way — retries steadily without firing the reducer every frame. */
...
export function createSelfController(deps: SelfControllerDeps) {
   ...
   *  against a wall, boulder, or tree with nowhere to slide. The probe carries the
  ...
  const driveSelf = (entry: MotionEntry, x: number, y: number, now: number) => {
    ...
    if (!sameMoveIntent(desired, sent)) {
      ...
      if (!canProgress(entry, x, y, desired)) {
        // Flush against a wall, boulder, or tree: stand facing it rather than store a
  ...
  const syncDestinationFromPath = (path: string) => {
    if (path === "") {
      // Keep the marker. An empty path can mean "no route right now" — an obstacle has
      // sealed the only way — and we keep trying toward the clicked tile rather than
      // abandoning it. The marker clears on arrival, on a keypress, or when a new tile is clicked.
      destinationPath = "";
      return;
    }
