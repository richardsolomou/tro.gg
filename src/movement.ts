 ...
 *  exists yet, so re-routing around an obstacle — or waiting for it to clear the only way —
...
export interface SelfControllerDeps {
  conn: DbConnection;
  /** Trogg collision bounds, for prediction and probes. */
  bounds: ZoneBounds;
...
export function createSelfController(deps: SelfControllerDeps) {
   ...
   *  against an obstacle with nowhere to slide. The probe carries the
  ...
  const driveSelf = (entry: MotionEntry, x: number, y: number, now: number) => {
    ...
    if (!sameMoveIntent(desired, sent)) {
      ...
      if (!canProgress(entry, x, y, desired)) {
        // Flush against an obstacle: stand facing it rather than store a
  ...
  const syncDestinationFromPath = (path: string) => {
    if (path === "") {
      // Keep the marker. An empty path can mean "no route right now" — an obstacle sealed
  ...
  const update = (entry: MotionEntry, motion: ProjectedMotion, now: number) => {
    ...
    // A click-to-move route stalls when an obstacle lands on a tile
