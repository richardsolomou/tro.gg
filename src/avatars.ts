...
const FRAME = 32;
/** Tile size the 32px frame was authored against (gen-sprites: frame over a 28px tile). */
const DESIGN_TILE = 28;
...
export class Avatar {
  ...
  constructor(set: SpriteSet, color: number, self: boolean, tile: number) {
    ...
    // Geometry is authored against DESIGN_TILE, then the whole view scales to the
    // viewport's current tile (TILE is dynamic since the terrain layer landed),
    // so the trogg keeps its proportions and grounding at any zoom.
    const t = DESIGN_TILE;
    ...
    const aura = new Graphics().ellipse(t / 2, t - 2, 8, 3).fill({ color, alpha: self ? 0.85 : 0.5 });
    if (self) aura.ellipse(t / 2, t - 2, 9, 4).stroke({ width: 1, color: 0xe8dcc4, alpha: 0.9 });

    this.sprite = new AnimatedSprite(set.down);
    this.sprite.anchor.set(16 / FRAME, 30 / FRAME); // bottom-centre rig anchor
    this.sprite.position.set(t / 2, t); // feet at the tile's base
    this.sprite.animationSpeed = 0.15; // ~9fps, reads as a brisk walk
    this.sprite.gotoAndStop(0);

    this.view.addChild(aura, this.sprite);
    this.view.scale.set(tile / DESIGN_TILE);
  }
